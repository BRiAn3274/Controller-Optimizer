...
-- 普通 Brimstone 保持四方向输出，布尔与浮点输入保持一致。
reset(1)
setStick(0.8, 0.6)
...
assertEqual(poll(InputHook.IS_ACTION_PRESSED, ButtonAction.ACTION_SHOOTRIGHT), true, "pressed right")
assertEqual(poll(InputHook.IS_ACTION_PRESSED, ButtonAction.ACTION_SHOOTDOWN), false, "pressed down")
...
assertEqual(poll(InputHook.IS_ACTION_PRESSED, ButtonAction.ACTION_SHOOTRIGHT), true, "analog pressed right")
assertEqual(poll(InputHook.IS_ACTION_PRESSED, ButtonAction.ACTION_SHOOTDOWN), true, "analog pressed down")
...
-- 回中保持期间，GET_ACTION_VALUE 与 IS_ACTION_PRESSED 同时维持方向；
-- 真正释放后只短暂覆盖输入，之后恢复 nil 以兼容其他 Mod。
reset(40)
setStick(1, 0)
assertEqual(poll(InputHook.GET_ACTION_VALUE, ButtonAction.ACTION_SHOOTRIGHT), 1, "hold acquire")
frame = 41
setStick(0, 0)
assertEqual(poll(InputHook.IS_ACTION_PRESSED, ButtonAction.ACTION_SHOOTRIGHT), true, "hold pressed consistency")
frame = 86
assertEqual(poll(InputHook.GET_ACTION_VALUE, ButtonAction.ACTION_SHOOTRIGHT), 0, "release guard value")
assertEqual(poll(InputHook.IS_ACTION_PRESSED, ButtonAction.ACTION_SHOOTRIGHT), false, "release guard pressed")
frame = 93
...
-- 真正回中只防抖 4 帧，随后自然释放；稳定回中后才允许下一次咳血。
frame = 204
setStick(0, 0)
...
-- 回中后的反向 snapback 不能变成第二次攻击，也不能改变已经锁定的方向。
reset(300)
player.playerType = PlayerType.PLAYER_AZAZEL_B
setStick(1, 0)
...
frame = 401
setStick(-0.6, 0.8)
assertClose(poll(InputHook.GET_ACTION_VALUE, ButtonAction.ACTION_SHOOTRIGHT), 0.8, 0.0001, "tainted analog direction locked x")
assertClose(poll(InputHook.GET_ACTION_VALUE, ButtonAction.ACTION_SHOOTDOWN), 0.6, 0.0001, "tainted analog direction locked y")
