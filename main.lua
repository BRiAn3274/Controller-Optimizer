...
local Config = {
    ...
    -- 真正释放后的短保护只覆盖几帧，避免长期返回 0 覆盖其他输入 Mod。
    OutputReleaseGuardFrames = 6,
    ...
    TaintedAzazelReleaseDebounceFrames = 4,
...
local function sanitizeConfig()
    Config.OutputReleaseGuardFrames = clampInteger(Config.OutputReleaseGuardFrames, 0, 15, 6)
    ...
    Config.TaintedAzazelReleaseDebounceFrames = clampInteger(Config.TaintedAzazelReleaseDebounceFrames, 1, 12, 4)
    ...
    ModConfigMenu.AddSetting(
        ...
        "General",
        {
            ...
            Info = {
                ...
                "Rebuilds one trigger followed by a stable held direction.",
...
local function finishShootRelease(state, frame, reason, suppressRaw)
    state.active = false
    state.x = 0
    state.y = 0
    state.releaseStartFrame = nil
...
-- 普通 Brimstone/Azazel 沿用已经验证有效的长保持路径，同时同步浮点和
-- pressed 输出。这里只稳定输入，不直接创建或修正激光实体。
local function refreshGenericBrimstoneState(state, controllerIndex, player)
    ...
    if magnitude >= Config.EnterDeadzone then
        ...
        -- 一旦摇杆进入回中区，就锁住最后意图方向。回中后的短暂反向尖峰
        -- 无论幅度多大都视为 snapback；持续超过保护窗口才视为玩家主动反向。
        local releaseAge = state.releaseStartFrame ~= nil and
            frame - state.releaseStartFrame or nil
        local releaseSnapback =
            state.active and
            releaseAge ~= nil and
        ...
        local springBackSpike =
            state.active and
            dot <= Config.ReverseGuardDot and
            (releaseSnapback or
                (framesSinceActive <= Config.ReverseGuardFrames and
                    magnitude <= Config.ReverseGuardMaxMagnitude))
    ...
    elseif state.active then
        ...
        if shouldUseChargeFamiliarReleasePath(player) and
            ...
            finishShootRelease(state, frame, "chargeFamiliarReleased", false)
            return
        end

        if framesSinceActive > Config.BeamHoldFrames then
            finishShootRelease(state, frame, "beamHoldExpired", false)
            appendDiagnosticLog(
...
-- 里阿撒泻勒的原生攻击依赖一次 triggered 启动咳血，随后由同一方向的
-- pressed/value 继续蓄力。状态机只重建这段输入序列，游戏仍负责全部攻击行为。
local function refreshTaintedAzazelState(state, controllerIndex, player)
    ...
    if state.active then
        ...
        if state.releaseStartFrame ~= nil then
            local releaseAge = frame - state.releaseStartFrame
            if magnitude >= Config.EnterDeadzone then
                local x = rawX / magnitude
                local y = rawY / magnitude
                local dot = (x * state.x) + (y * state.y)

                -- 同方向重新出现说明只是采样掉帧；反向值则更可能是回中回弹，
                -- 保持释放计时且绝不把它变成新的攻击方向。
                if dot > Config.ReverseGuardDot then
                    state.releaseStartFrame = nil
                    state.lastActiveFrame = frame
                    state.releaseReason = "taintedChargeRecovered"
                elseif releaseAge >= Config.TaintedAzazelReleaseDebounceFrames then
                    state.taintedCenterStartFrame = nil
                    finishShootRelease(state, frame, "taintedReleasedAfterSnapback", true)
                end
            elseif releaseAge >= Config.TaintedAzazelReleaseDebounceFrames then
                state.taintedCenterStartFrame = state.releaseStartFrame
                finishShootRelease(state, frame, "taintedReleased", true)
            end
        elseif magnitude < Config.EnterDeadzone then
            state.releaseStartFrame = frame
...
local function isRecentFilteredRelease(state)
    return Isaac.GetFrameCount() - state.lastReleaseFrame <= Config.OutputReleaseGuardFrames
end
...
local function handleGenericShootInput(state, inputHook, buttonAction)
    if inputHook == InputHook.IS_ACTION_TRIGGERED or isActionPressedHook(inputHook) then
        ...
        if isActionPressedHook(inputHook) then
            if state.active then
                return (getShootOutput(state, buttonAction) or 0) > 0.001
            end
            if isRecentFilteredRelease(state) and state.rawMagnitude < Config.EnterDeadzone then
                return false
            end
        end
        return nil
